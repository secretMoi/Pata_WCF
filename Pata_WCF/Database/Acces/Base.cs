...
namespace Database.Acces
{
	public class Base
	{
		...
		protected Pata_WCF.Database.Classes.Base _classesBase;
		#endregion
		...
		public List<Pata_WCF.Database.Classes.Base> Lire(string index)
		{
			CreerCommande("Lire");
			...
			List<Pata_WCF.Database.Classes.Base> liste = AssigneChamp(sqlDataReader);
		...
		public Pata_WCF.Database.Classes.Base LireId(int id)
		{
			...
			Pata_WCF.Database.Classes.Base element = AssigneChamp(sqlDataReader)[0];
		...
		/// </summary>
		/// <returns>Une liste des enregistrements trouvés</returns>
		protected List<Pata_WCF.Database.Classes.Base> AssigneChamp(SqlDataReader sqlDataReader)
		{
			Type type = _classesBase.GetType(); // récupère le type de l'objet

			List<Pata_WCF.Database.Classes.Base> liste = new List<Pata_WCF.Database.Classes.Base>(); // crée la liste à retourner
			...
			while (sqlDataReader.Read()) // parcourt toutes les données SQL reçues
			{
				...
				liste.Add(classe as Pata_WCF.Database.Classes.Base); // ajoute l'objet à la liste de retour
			}
